...
export default class LoginScreen extends React.Component {
  ...
  onPress () {
    this.props.navigation.reset({
      index: 0,
      routes: [{ name: 'Home', params: this.state.email }],
    })
  }
  render(){
    return(
        ...
        <View style={styles.container}>
          <View style={styles.inputForm}>
            ...
            <View style={styles.submitViewCont}>
              <TouchableOpacity onPress={()=>this.onPress()} activeOpacity={1} disabled={!validateForm(this.state)}>
                  <View style={styles.submitCont}>
                    <Text style={!validateForm(this.state) ? styles.submitText : [styles.submitText,{color: '#fff'}]}>Login</Text>
                  </View>
              </TouchableOpacity>
            </View>
            <TouchableOpacity onPress={()=>this.props.navigation.navigate('Register')} activeOpacity={1} >
